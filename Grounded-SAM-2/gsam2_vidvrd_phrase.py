...
def process_video(VIDEO_PATH, TEXT_PROMPT, OUTPUT_VIDEO_PATH, SOURCE_VIDEO_FRAME_DIR, SAVE_TRACKING_RESULTS_DIR, gt_bbox, tube_start_frame, tube_end_frame, category, width, height):
    """
    ...
    vIoU = run_coco_eval_per_image_boxes(prediction, gt_bbox, image_size=(width, height), category_name=category)
    print("vIoU: ", vIoU)
...
def process_video_data(json_data, stop_num):
    ...
    for video in json_data:
        ...
        TEXT_PROMPT = caption
        OUTPUT_VIDEO_PATH = f"./vidvrd_eval_result/phrase/video_output/{video_ID}_tracking_demo.mp4"
        SOURCE_VIDEO_FRAME_DIR = f"./vidvrd_eval_result/phrase/video_frames/{video_ID}"
        SAVE_TRACKING_RESULTS_DIR = f"./vidvrd_eval_result/phrase/tracking_results/{video_ID}"

        if not os.path.exists(SAVE_TRACKING_RESULTS_DIR):
            os.makedirs(SAVE_TRACKING_RESULTS_DIR)
        
        vIoU = process_video(VIDEO_PATH, TEXT_PROMPT, OUTPUT_VIDEO_PATH, SOURCE_VIDEO_FRAME_DIR, SAVE_TRACKING_RESULTS_DIR, bbox, tube_start_frame, tube_end_frame, category, width, height)
        ...
        if id_count %100 == 0:
            ...
            with open('/home/we337236/stvg/Grounded-SAM-2/vidvrd_eval_result/phrase/log/vIoU_list.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    data = read_json_file("/home/we337236/stvg/dataset/vidvrd/referral_final_output.json")
