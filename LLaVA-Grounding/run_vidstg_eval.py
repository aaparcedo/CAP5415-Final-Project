...
TEMP_IMAGE_PATH = "./temp_llava_frame_vidstg.jpg"
...
def evaluate_entry_llava(frames_with_gt, entry, runner, device="cuda"):
    ...
    for frame, gt_bbox in frames_with_gt:
        ...
        if boxes is not None and len(boxes) > 0:
            ...
            w_box = x2_abs - x1_abs
            h_box = y2_abs - y1_abs
            predicted_boxes.append([x1_abs, y1_abs, w_box, h_box])
        else:
            predicted_boxes.append([0, 0, 0, 0])

        ground_truth_boxes.append(gt_bbox)

    ious = [calculate_iou(pred, gt) for pred, gt in zip(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_path", type=str, default="./results/vidstg_llava_predictions.json",
