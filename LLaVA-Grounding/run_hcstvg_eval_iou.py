...
# HC-STVG annotation file
INPUT_JSON = "/home/da530038/groudingdino/GroundingDINO/samples.json"
VIDEO_DIR = "/home/da530038/datasets_download/something_else/20bn-something-something-v2"
SAVE_RESULTS_PATH = "./results/video_batch_predictions_llava.json"
TEMP_IMAGE_PATH = "./temp_llava_frame_hcstvg1_iou.jpg"
...
def unresize_boxes(box_list, img_width, img_height):
    ...
    box_tensor = torch.cat(box_list, dim=0).clone()  # shape: [N, 4]
...
def evaluate_entry_llava(frames, entry, runner, device, box_threshold=0.3, frame_step=15):
    ...
    for i, frame in enumerate(frames[::frame_step]):
        frame_idx = i  # index into sampled_gt_boxes
        ...
        if boxes is not None and len(boxes) > 0 and frame_idx < len(sampled_gt_boxes):
            corrected_tensor = unresize_boxes(boxes, frame_W, frame_H)  # shape [N, 4]
            corrected_boxes = corrected_tensor.cpu().numpy()
            gt_box = sampled_gt_boxes[frame_idx]  # [x, y, w, h]
            print(f"Frame:{frame_idx}")
            best_iou = -1
            best_box = [0, 0, 0, 0]

            for b in corrected_boxes:
                # Convert normalized [x1, y1, x2, y2] to absolute [x, y, w, h]
                x1 = b[0] * frame_W
                y1 = b[1] * frame_H
                x2 = b[2] * frame_W
                y2 = b[3] * frame_H
                abs_box = [x1, y1, x2 - x1, y2 - y1]

                score = calculate_iou(abs_box, gt_box)
                if score > best_iou:
                    best_iou = score
                    best_box = abs_box

            frame_box = [round(v, 2) for v in best_box]
        else:
            print("frame box 0 returned")
            frame_box = [0, 0, 0, 0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_path", type=str, default="./results/referral_hcstvg1_iou_llava_predictions.json",
